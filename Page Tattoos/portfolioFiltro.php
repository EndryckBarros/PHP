...
<?php
	...
	if($idCategoria != '' && $idArtista == ''){
		$sql = "SELECT * FROM postagem WHERE Categoria_idCategoria = $idCategoria";
	         $result = mysqli_query($conn, $sql);
	        while($row = mysqli_fetch_assoc($result)){ 
	           	$posts[$cont] = $row['ImagemLink'];
	           	$cont += 1;
	        }
	}

	if($idArtista != '' && $idCategoria == ''){
		$sql = "SELECT * FROM postagem WHERE Artista_idArtista = $idArtista";
	         $result = mysqli_query($conn, $sql);
	        while($row = mysqli_fetch_assoc($result)){ 
	           	$posts[$cont] = $row['ImagemLink'];
	           	$cont += 1;
	...
	if($idArtista != '' && $idCategoria != ''){
		$sql = "SELECT * FROM postagem WHERE Categoria_idCategoria = $idCategoria
		AND Artista_idArtista = $idArtista";
	         $result = mysqli_query($conn, $sql);
	        while($row = mysqli_fetch_assoc($result)){ 
	           	$posts[$cont] = $row['ImagemLink'];
	           	$cont += 1;
...
<?php
	for ($i=0; $i < $cont; $i++) { 
	                ...
	                $sql = "SELECT * FROM postagem WHERE ImagemLink = '$posts[$i]'";
                    $result = mysqli_query($conn, $sql);
                    while($row = mysqli_fetch_assoc($result)){
                    	$tituloCard = $row['titulo']; 
                        $legendaCard =  $row['Legenda']; }?>
			...
			<div class="mdl-cell--3-col mdl-cell--4-col-tablet mdl-cell--4-col-phone">
				<div>
					<div class="mdl-card  mdl-shadow--4dp demonstracao-card">
						...
						<?php $caminho = "../materialize/img/" . $posts[$i]; ?>

						<div class="mdl-card__title" style="background: 
							url('<?php echo $caminho ?>') center / cover;">
						...
						<form method="post">
							<input type="hidden" name="postagem" value="<?php echo $posts[$i]?>" />
							<button class="mdl-button mdl-js-button mdl-js-ripple-effect mdl-button--accent">Agendar</button>
						</form>
						</div>
	...
	<?php
	} include("controller/controller.php"); agendarServico(); ?>
